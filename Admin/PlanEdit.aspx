<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/AdminMaster.master" AutoEventWireup="true" CodeFile="PlanEdit.aspx.cs" Inherits="Admin_PlanEdit" ClientIDMode="Static" %>
    ...
    <script>
        $(document).ready(function () {
            ...
            $("#btnSub").click(function (e) {
                var Benefitcount = 0, SumInsured = 0, Deductible = 0;
        ...
        function GetSelectedRow(lnk) {
            ...
            $.ajax({
                url: '<%= ResolveUrl("PlanEdit.aspx/GetPlanDetails") %>',
                 type: 'POST',
                 dataType: 'json',
                 contentType: 'application/json; charset=utf-8',
                 data: '{Plan_Code:' + JSON.stringify(PlanCode) + '}',
                 success: function (response) {
                     if (response.d != '') {
                         var data = response.d.split('#');
                         if (data[0] == 'F') {

                             $('#lbl_PlanCode').text(data[1].toString());
                             $('#txtPlanName').val(data[2].toString());

                             $('#txtMinAge').val(data[4].toString());
                             $('#txtMaxAge').val(data[5].toString());
                             $('#txtmintripDuration').val(data[6].toString());
                             $('#txtmaxDuration').val(data[7].toString());
                             var rdo = data[8].toString();
                             var radio = $("[id*=RdbValidFor] label:contains('" + rdo + "')").closest("td").find("input");
                             radio.attr("checked", "checked");

                             //$('#ddlValueaddedservices input').each(function () {
                             //    alert($('#ddlValueaddedservices').val());
                             //    if (this.val == 'Luggage Tracker') {
                             //        $(this).attr('checked', 'checked');
                             //    };
                             //});
                            
                             var values = data[3].toString().split(',');
                             var listb = document.getElementById('ddlValueaddedservices');
                             //var VASlength = $('#ddlValueaddedservices > option').length;        
                             var VASlength = listb.options.length;
                            for (var i = 0; i < VASlength; i++)
                            {
                                //$("#ddlValueaddedservices option:selected").prop("selected", false)
                                //
                                for (var x = 0; x < values.length; x++)
                                {
                                    //alert(listb.options[i].text);
                                    if (listb.options[i].text == values[x])
                                    {
                                        alert(1);
                                        //$(listb.options[i].text).prop("selected", true);
                                        $("#ddlValueaddedservices option[item='" + listb.options[i].text + "']").attr("selected", "true");
                                        //$("#ddlValueaddedservices option:selected").prop("selected", true)
                                       // listb.options[i].selected = true;
                                    }
                                }
                            }

                            var text1 = data[9].toString();
                            $("#ddlTAndC option").filter(function () {
                                    ...
                                    <Columns>
                                        <asp:TemplateField HeaderText="S.No.">
                                            <ItemTemplate>
                                                ...
                                                <asp:HiddenField ID="hdnPlanCode" runat="server" Value='<%#Bind("Plan_Code") %>'></asp:HiddenField>
                                            </ItemTemplate>
                                        </asp:TemplateField>
                                        ...
                                        <asp:TemplateField HeaderText="Edit">
                                            <ItemTemplate>
                                                <asp:Button ID="btnEdit" runat="server" Text="Edit" class="btn btn-sm round btn-outline-danger EditPlan" CommandName="EditCommand"></asp:Button>
                                            </ItemTemplate>
                                        </asp:TemplateField>
                                ...
                                <div class="card">
                                    <div class="card-content collapse show">
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-4">
                                                        <div class="form-group">
                                                            ...
                                                            <asp:Label ID="lbl_PlanCode" runat="server" class="form-control"></asp:Label>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="col-md-4">
                                                        <div class="form-group">
                                                            ...
                                                            <asp:TextBox ID="txtPlanName" runat="server" class="form-control" placeholder="Enter Plan Name" MaxLength="50"></asp:TextBox>
                                                        </div>
                                                    </div>
                                        ...
                                        <div class="card-body">
                                            ...
                                            <div class="form-body">
                                                ...
                                                <div class="table-responsive" id="Div_Geo" runat="server">
                                                    ...
                                                    <%--                                                    <asp:Table runat="server" ID="maindiv">
                                        ...
                                        <div class="footer" align="right">
                                            ...
                                            <asp:Button ID="btnSub" runat="server" Text="Submit" CssClass="btn btn-primary edit-bg create-user" />
                                            <%--  <asp:Button ID="BtnReset" runat="server" Text="Close" CssClass="btn btn-primary edit-bg create-user" />--%>
